...
\mpierre{Note that the number {\tt b} is not stored in the binary vectors, 
but it is stored here.}
\pierre{The order of arguments was not the same here as in the allocate, 
  so I changed it here.  Beware!}  
\endtab
\code
...
on each of the first {\tt nl} rows of the matrix {\tt m1}.
 \pierre{Seems very specialized.  Why is this useful? }
 \endtab
\code
...
Only the first submatrices are considered. \pierre{Meaning?  Sizes must be compatible!}
\endtab
\code
...
\pierre{Make this clearer.  Return in a character string?}
 \endtab
\code
