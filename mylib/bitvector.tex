...
\hpierre{In Boost, they start from the least significant bit of the integer.}
\pierre{We may add functions to compute the Hamming weight (number of ones)
  and perhaps other tools commonly available for bit vectors.}
...
string bitvector_display (bitvector_vector *v, int b);
\endcode
 \tab
Returns a character string that contains the first {\tt b} bits of the bit vector {\tt v}.
 \endtab\code

void bitvector_displayMathematica (bitvector_vector *b, int v);
\endcode
 \tab
Returns a character string that contains the first {\tt b} bits of the bit vector {\tt v},
in a format compatible for Mathematica input.   \pierre{Needed?}
 \endtab
\code
